...
(
    ...
    FOREIGN KEY (company_id) REFERENCES migration.companies(id) ON UPDATE CASCADE ON DELETE CASCADE
);

DROP TABLE IF EXISTS migration.patients_records;
CREATE TABLE migration.patients_records
(
    LIKE patients_records INCLUDING ALL
);

DROP TABLE IF EXISTS migration.medical_record_entries;
CREATE TABLE migration.medical_record_entries
(
    LIKE medical_record_entries INCLUDING ALL,

    FOREIGN KEY (patient_id) REFERENCES migration.patients_records(id) ON UPDATE CASCADE ON DELETE CASCADE
);
